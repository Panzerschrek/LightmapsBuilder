...
void main()
{
	g_tex_coord= vec3( tex_coord, float(texture_layer) + eps );
	g_texture_array= float(texture_array) + eps;

	g_lightmap_coord= vec3( lightmap_coord, float(lightmap_layer) + eps );
